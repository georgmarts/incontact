...
export default function FriendProfile(props: Props) {
  ...
  async function deleteFriend() {
    ...
    const { error } = await supabase
      ...
      .delete()
      .or(
        `and(userId.eq.${session.user.id},friendId.eq.${props.friendId}),and(userId.eq.${props.friendId},friendId.eq.${session.user.id})`
      );
    if (!error) {
      router.push("/friends");
      setNotification(
        `${friend?.firstName} ${friend?.lastName} is deleted from your friends list`
      );
      closeOptionsModal();
    }
    if (error) {
  ...
  return (
    <>
      <dialog ref={optionsModalRef} className="friend-profile__options">
        <Button
          label={`Delete ${friend.firstName}`}
          color="#ff0084"
          fn={deleteFriend}
        />
        <Button label="Cancel" color="grey" fn={closeOptionsModal} />
      </dialog>
      <div className="friend-profile">
        <img
          src="/icons/dots.svg"
          alt=""
          className="friend-profile__dots"
          onClick={openOptionsModal}
        />
        <ProfileHeader user={friend}>
          <Link href={`/messages/${props.friendId}`} style={{ all: "unset" }}>
