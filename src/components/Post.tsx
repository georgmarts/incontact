...
import { isShareModalOpen, notificationMessage, shareInfo, updateCommentsCountAtom } from '@/app/state/jotai'
...
export default function Post({post, user}: Props) {
  ...
  async function handleDeletePost() {
    setIsLoading(true)
    const { error: deletePostError } = await supabase.from('SNPosts').delete().match({id: post.id})
    const {error: deleteCommentsError} = await supabase.from('SNComments').delete().match({post: post.id})
    setIsLoading(false)
    if(post.img) {
      setIsLoading(true)
    ...
    <div className='post'>
        <div className='post__user-card'>
          ...
          <dialog className='post__user-card--options-modal' ref={postModalRef}>
            {isLoading ? <h4>Deleting post...</h4> : <>
            <Button label='Edit post' width='100%' fn={openEditModal} loading={isLoading}/>
            <Button label='Delete post' width='100%' color='#ff0084' fn={handleDeletePost} loading={isLoading}/>
            <Button label='Cancel' color='#c1b9b0' fn={() => setPostModal(false)} width='100%' loading={isLoading}/>
            </>}
          </dialog>
        </div>
        <p className='post__body'>{post.body}</p>
        <dialog className='post__edit-modal' ref={editModalRef}>
        {isLoading ? <h4>Saving changes...</h4> :
        <form onSubmit={handleEditPost}>
            <textarea value={postBody} rows={3} onChange={(e) => setPostBody(e.target.value)} disabled={isLoading}></textarea>
            <Button label='Submit' width='80%' loading={isLoading}/>
            <Button label='Cancel' width='80%' color='#c1b9b0' fn={() => setEditModal(false)} type='button' loading={isLoading}/>
          </form>}
        </dialog>
        <img src={post.img} alt="" className='post__img'/>
        <div className='post__icons'>
          <img src={isLiked ? "/icons/like.svg" : "/icons/unlike.svg"} alt="" width='30px' onClick={isLiked ? handleDislike : handleLike}/>
