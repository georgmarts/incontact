...
export default function Comment({postId, comments, parentComment, fetchComments}: Props) {
    ...
    <div className='comment'>
        <div className='comment__user'>
            <UserPhoto image={parentComment.SNUsers?.img} height='30px'/>
            <p>{parentComment.SNUsers?.firstName} {parentComment.SNUsers?.lastName}</p>
            <p>{time}</p>
        </div>
        ...
        {isReply && <div className='comment__reply'>
            ...
            <Button label='C' width='50px' fn={handleCancelReply}/>
        </div>}
        {childrenComments.length > 0 && <div className='comment__children'>
