  ...
  return (
    <main className="friends-page">
      ...
      <Header>
        <h3>Friends</h3>
        {friends && (
          <span className="friends-page__header--count">
            ({friends?.length})
          </span>
        )}
      </Header>
      ...
      {friends ? (
        <p className="friends-page__tag">
          {friends?.length > 0 ? "Friends list" : "You do not have friends"}
        </p>
      ) : (
        <div
          style={{
      ...
      <List>
        {!friends
          ? friendsListSkeleton.map((friend, i) => <FriendSkeleton key={i} />)
          : friends.map((user, i) => <Friend user={user} key={i} />)}
      </List>
    </main>
  );
}
