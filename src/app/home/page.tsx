...
export default function Page({}: Props) {
    ...
    channel
      .on('presence', { event: 'sync' }, () => {
          ...
          .map((presenceId) => {
            ...
            return presences.map((presence) => {return presence.id})
          })
          .flat()
  ...
  async function fetchPosts() {
    if(!friendIds) return
    const {data, error} = await supabase.from('SNPosts').select('*, SNUsers!SNPosts_user_fkey(*)').in('user', friendIds).order('createdAt', {ascending: false})
    if(data) {
      setPosts(data)
    } 
      ...
      <div className='home__friend-avatars'>
          {friends.map((friend, i) => 
            <div key={i} onClick={() => router.push(`/messages/${friend.id}`)}>
              <UserPhoto image={friend.img} height='60px' isOnline={onlineUsers.find(userId => userId == friend.id) ? true : false}/>
            </div>
          )}
      </div>
      {posts.length == 0 ?
